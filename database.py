# File: database.py (VERSI FINAL untuk Aiven)
...
import sys
import logging
import certifi # <-- Penting untuk koneksi SSL ke Aiven

# --- Fungsi Konfigurasi dan Koneksi (Disesuaikan untuk Aiven) ---
...
def create_connection():
    ...
    conn = None
    try:
        # Menggunakan file sertifikat SSL yang sudah diunduh
        base_path = get_base_path()
        ssl_ca_path = os.path.join(base_path, 'ca.pem')

        if not os.path.exists(ssl_ca_path):
            error_msg = f"File sertifikat 'ca.pem' tidak ditemukan di: {ssl_ca_path}. Unduh dari dashboard Aiven dan letakkan di folder proyek."
            logging.error(error_msg)
            raise FileNotFoundError(error_msg)
        ...
        conn = mysql.connector.connect(
            ...
            # --- Bagian Penting untuk Aiven ---
            ssl_ca=ssl_ca_path,
            ssl_verify_cert=True
            # ----------------------------------
        )
        logging.info("Koneksi ke Aiven berhasil.")
    except Error as e:
...
def create_tables():
    ...
    try:
        ...
        if not (conn and conn.is_connected()):
            logging.error("Tidak bisa membuat tabel, koneksi database gagal.")
            return
...
def validate_admin(personal_number, password):
    ...
    try:
        ...
        if conn and conn.is_connected():
            ...
            if result:
                logging.info(f"User '{personal_number}' ditemukan di DB. Membandingkan password.")
                stored_password_hash = result['password'].encode('utf-8')
                if bcrypt.checkpw(password.encode('utf-8'), stored_password_hash):
                    logging.info("Password MATCH. Login berhasil.")
                    user = True
                else:
                    logging.warning("Password MISMATCH. Login gagal.")
            else:
                logging.warning(f"User '{personal_number}' TIDAK ditemukan di DB.")
    except (Error, KeyError) as e:
...
def add_item(name, desc, stock, cat_id):
    try:
        ...
        if conn and conn.is_connected():
            ...
            query = "INSERT INTO items (item_name, description, stock, date_added, category_id) VALUES (%s, %s, %s, %s, %s)"
            cursor.execute(query, (name, desc, stock, date, cat_id))
            conn.commit()
            cursor.close()
            conn.close()
    except (Error, KeyError) as e:
...
def get_all_items_with_category():
    ...
    try:
        ...
        if conn and conn.is_connected():
            cursor = conn.cursor()
            query = "SELECT i.item_id, i.item_name, c.category_name, i.stock, i.description, i.date_added FROM items i LEFT JOIN categories c ON i.category_id = c.category_id ORDER BY i.item_name"
...
def update_item(item_id, name, desc, stock, cat_id):
    try:
        ...
        if conn and conn.is_connected():
            cursor = conn.cursor()
            query = "UPDATE items SET item_name = %s, description = %s, stock = %s, category_id = %s WHERE item_id = %s"
            cursor.execute(query, (name, desc, stock, cat_id, item_id))
            conn.commit()
            cursor.close()
            conn.close()
    except (Error, KeyError) as e:
